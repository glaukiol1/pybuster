    ...
    def sub():
        ...
        for word in words:
            if pipe['run'] == False:
                break
            try:
                response = requests.get(outstr.replace("PYBUSTER",word))
                if _s.find(str(response.status_code))!=-1:
                    pipe['found'] = pipe['found'] + 1
                    pipe['dirs'].append(word)
                    print(f'= {outstr.replace("PYBUSTER",word)} {" "*(33-(outstr.replace("PYBUSTER",word).__len__()+str(id).__len__()))}(Status: {response.status_code}) (Found by: #{id})  =')
            except:
                pipe['errors'] = pipe['errors'] + 1
            pipe['total_done'] = pipe['total_done'] + 1
    if mode == 'dir':
